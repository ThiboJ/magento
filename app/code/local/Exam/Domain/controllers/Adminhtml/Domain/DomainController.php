...
class Exam_Domain_Adminhtml_Domain_DomainController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        return $this->loadLayout()->_setActiveMenu('exam_domain');
    }
    ...
    public function editAction()
    {
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('exam_domain')->__('Domain does not exist'));
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                $this->_getSession()->addError($e->getMessage());
                $this->_getSession()->addException($e, Mage::helper('exam_domain')->__('An error occurred while saving the domain.'));
            }
    ...
    public function deleteAction()
    {
        if ($id = $this->getRequest()->getParam('id')) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('exam_domain')->__('Domain was successfully deleted'));
                $this->_redirect('*/*/');
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($domainIds)) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('exam_domain')->__('Please select domain(s)'));
        } else {
            try {
                foreach ($domainIds as $domain) {
                    Mage::getModel('exam_domain/domain')->load($domain)->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('exam_domain')->__('Total of %d domain(s) were successfully deleted', count($domain)));
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            }
        }
    ...
    public function massStatusAction()
    {
        ...
        if (!is_array($domainIds)) {
            ...
            try {
                foreach ($domainIds as $domain) {
                    Mage::getSingleton('exam_domain/domain')->load($domain)->setIsActive($this->getRequest()->getParam('is_active'))->setIsMassupdate(true)->save();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('exam_domain')->__('Total of %d domain(s) were successfully updated', count($domainIds)));
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            }
        }
    ...
    protected function _saveImage($imageAttr, $delete)
    {
        if ($delete) {
            ...
            $model = Mage::getModel('exam_domain/domain')->load($this->getRequest()->getParam('id'));
